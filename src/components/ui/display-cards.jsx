...
export default function DisplayCards({ cards, onCardClick }) {
  ...
  return (
    <div className="grid [grid-template-areas:'stack'] place-items-center opacity-100 animate-in fade-in-0 duration-700 pointer-events-auto">
      {displayCards.map((cardProps, index) => (
        <DisplayCard key={index} {...cardProps} onClick={() => onCardClick && onCardClick(index)} />
      ))}
    </div>
  );
}
